...
(function (root, factory) {
  ...
  function metricKind(key, label) {
    ...
    if (/cpa|cost per|average gift/.test(n)) return "usd2";
    if (/^ntl$|names|count|recipients|donors|unsubs?$/.test(n)) return "count";
    return "usd";
  }
  ...
  function extractP2P(sheet, opts) {
    ...
    if (unresolvable.length) {
      warn("Could not work out how to combine “" + unresolvable.map(k => metricLabels[k]).join(", ") +
           "” across rows (no stored numerator for it), so it is omitted from monthly and total " +
           "rollups rather than averaged.");
    }
  ...
  /* ===================================================================
     ...
     =================================================================== */
  function reconcile(dig, paid, p2p) {
    ...
    if (p2p) {
      ...
      checks.forEach(([rk, nk, dk, label, desc]) => {
        ...
        p2p.detail.forEach(r => {
          ...
          n++;
          if (!near(ratio, num / den, Math.max(0.02, Math.abs(num / den) * 1e-6))) bad++;
        });
        if (bad) problems.push({level: "error",
          text: bad + " of " + n + " " + label + " cells on the P2P Statistics tab do not equal " + desc + "."});
  ...
  function extractAll(book, opts) {
    ...
    const digProblems = [], paidProblems = [], p2pProblems = [];
    ...
    let dig = null, paid = null, p2p = null;
    if (ds) {
    ...
    const rec = reconcile(dig, paid, p2p);
